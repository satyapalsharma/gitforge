...
export async function createRepo(accessToken, name, description) {
  ...
  try {
    ...
    if (
      ...
      error.message.includes('Repository creation failed')
    ) {
      console.log(`[GitHub] Repo ${name} might already exist. Fetching existing repo...`);
      try {
 ...
 * @param {string} date - ISO date string for backdating
 * @param {string} email - GitHub verified email
 * @param {boolean} [simulatePRs=false] - Whether to simulate a PR merge
 * @returns {Promise<object>} Created commit data
 */
export async function createBackdatedCommit(accessToken, owner, repo, files, message, date, email, simulatePRs = false) {
  const repoBase = `/repos/${owner}/${repo}`;

  // 1. Get the latest commit SHA from refs/heads/main (with retry for propagation delay)
  let ref;
  for (let refAttempt = 0; refAttempt < 5; refAttempt++) {
    try {
      ref = await githubFetch(accessToken, `${repoBase}/git/refs/heads/main`);
      break;
    } catch (err) {
      if (refAttempt < 4 && err.message.includes('Not Found')) {
        console.warn(`[GitHub] refs/heads/main not found yet, retrying in ${(refAttempt + 1) * 2}s (attempt ${refAttempt + 1}/5)...`);
        await new Promise(r => setTimeout(r, (refAttempt + 1) * 2000));
      } else {
        throw err;
      }
    }
  ...
  if (!ref) {
    throw new Error('Could not resolve refs/heads/main after 5 attempts');
  }
  ...
  // 6. Update the ref
  if (simulatePRs) {
    const branchName = `feature/${message.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 30)}-${Date.now()}`;
    
    // Create the branch ref pointing to the original latestCommitSha
    await githubFetch(accessToken, `${repoBase}/git/refs`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    // Update the feature branch ref to point to the new commit
    await githubFetch(accessToken, `${repoBase}/git/refs/heads/${branchName}`, {
      method: 'PATCH',
      body: JSON.stringify({
        sha: newCommit.sha,
      }),
    ...
    const pr = await githubFetch(accessToken, `${repoBase}/pulls`, {
      method: 'POST',
      body: JSON.stringify({
        title: message,
        head: branchName,
        base: 'main',
        body: `Automated PR for: ${message}\n\nThis PR was generated to simulate a collaborative workflow.`,
      }),
    });

    // Merge the Pull Request
    await githubFetch(accessToken, `${repoBase}/pulls/${pr.number}/merge`, {
      method: 'PUT',
      body: JSON.stringify({
    ...
    console.log(`[GitHub] Backdated commit pushed via PR #${pr.number} and merged.`);
  } else {
