 ...
 */
export function scheduleCommits(startDate, endDate, totalCommits) {
  ...
  const weights = days.map((day) => {
    ...
    // Base weight: weekdays get higher weight
    let weight = isWeekend ? 0.3 : 0.7;
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    // Spread commits between 9 AM (hour 9) and 11 PM (hour 23)
    const startHour = 9;
    const endHour = 23;
