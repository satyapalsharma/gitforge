...
export async function validateApiKey(apiKey) {
  try {
    await geminiFetch(apiKey, 'gemini-3.5-flash', ':generateContent', {
      contents: [
        {
          parts: [{ text: 'Say "ok"' }],
        },
 ...
 */
export async function generateProjectStructure(apiKey, projectName, description, techStack, complexity = 'medium') {
  const model = complexity === 'very-complex' ? 'gemini-3.1-pro' : 'gemini-3.5-flash';
 ...
 */
export async function generateFileContent(apiKey, projectName, filePath, projectContext, complexity = 'medium') {
  const model = complexity === 'very-complex' ? 'gemini-3.1-pro' : 'gemini-3.5-flash';
...
export async function estimateTokens(apiKey, prompt) {
  const response = await geminiFetch(apiKey, 'gemini-3.5-flash', ':countTokens', {
    contents: [{ parts: [{ text: prompt }] }],
  });
...
export async function generateCode(apiKey, prompt) {
  const response = await geminiFetch(apiKey, 'gemini-3.5-flash', ':generateContent', {
    contents: [{ parts: [{ text: prompt }] }],
    generationConfig: {
      temperature: 0.2,
